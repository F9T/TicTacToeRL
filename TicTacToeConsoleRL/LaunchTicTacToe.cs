...
namespace TicTacToeConsoleRL
{
    public class LaunchTicTacToe
    {
        public static void Main(string[] _args)
        {
            ...
            AbstractPlayer player2 = new LearnPlayer(EnumSymbol.Player2, 0.95);
            ...
            {
                Save((LearnPlayer) player1);
            }
            if (player2 is LearnPlayer)
            {
                Save((LearnPlayer) player2);
            }
            Console.ReadLine();
        }
        ...
        private static LearnPlayer Load()
        {
            LearnPlayer player = null;
            var formatter = new BinaryFormatter();
            using (var stream = new FileStream("test.ser", FileMode.Open, FileAccess.Read))
            {
                try
                {
        ...
        private static void Save(LearnPlayer _player)
        {
            var formatter = new BinaryFormatter();
            using (var stream = new FileStream("test.ser", FileMode.Create, FileAccess.Write))
            {
                try
                {
